...
class MidiInfoCommandTest
{
    ...
    private static java.io.File createRichMidi(Path tempDir, String name) throws Exception
    {
        ...
        MetaMessage title = new MetaMessage();
        title.setMessage(0x03, titleData, titleData.length);
        t.add(new MidiEvent(title, 0L));

        byte[] cpData = "(c) 1991 LucasArts".getBytes(StandardCharsets.UTF_8);
        MetaMessage copyright = new MetaMessage();
        copyright.setMessage(0x02, cpData, cpData.length);
        t.add(new MidiEvent(copyright, 0L));

        byte[] instrData = "Piano".getBytes(StandardCharsets.UTF_8);
        MetaMessage instr = new MetaMessage();
        instr.setMessage(0x07, instrData, instrData.length);
        t.add(new MidiEvent(instr, 0L));

        byte[] lyricData = "Hello world".getBytes(StandardCharsets.UTF_8);
        MetaMessage lyric = new MetaMessage();
        lyric.setMessage(0x05, lyricData, lyricData.length);
    ...
    @Test
    void textOutput_durationAndTotalPresent(@TempDir Path tmp) throws Exception
    {
    ...
    @Test
    void textOutput_richMetaShown(@TempDir Path tmp) throws Exception
    {
    ...
    @Test
    void textOutput_noMetadata_showsPlaceholder(@TempDir Path tmp) throws Exception
    {
    ...
    @Test
    void textOutput_twoFiles_totalCountCorrect(@TempDir Path tmp) throws Exception
    {
    ...
    @Test
    void textOutput_failedFilesExcludedFromTotal(@TempDir Path tmp) throws Exception
    {
    ...
    @Test
    void csvOutput_headerRowPresent(@TempDir Path tmp) throws Exception
    {
        java.io.File f = createMidi(tmp, "song.mid");
    ...
    @Test
    void csvOutput_dataRowCount(@TempDir Path tmp) throws Exception
    {
        java.io.File f = createMidi(tmp, "song.mid");
        RunResult r = run("midi-info", "--format", "csv", f.toString());
    ...
    @Test
    void csvOutput_durationFieldIsNumeric(@TempDir Path tmp) throws Exception
    {
        java.io.File f = createMidi(tmp, "song.mid");
        RunResult r = run("midi-info", "--format", "csv", f.toString());
    ...
    @Test
    void tsvOutput_usesTabSeparator(@TempDir Path tmp) throws Exception
    {
        java.io.File f = createMidi(tmp, "song.mid");
    ...
    @Test
    void csvOutput_noTotalLine(@TempDir Path tmp) throws Exception
    {
        java.io.File f = createMidi(tmp, "song.mid");
        RunResult r = run("midi-info", "--format", "csv", f.toString());
    ...
    @Test
    void unknownFormat_exits2(@TempDir Path tmp) throws Exception
    {
        java.io.File f = createMidi(tmp, "song.mid");
    ...
    @Test
    void exitCode1_onEmptyDirectory(@TempDir Path tmp)
    {
