...
class PlaylistPlayerTest
{
    ...
    static class MockMidiProvider implements MidiOutProvider
    {
        ...
        {
            return List.of(new MidiPort(0, "Mock"));
        }
    ...
    static class MockPlaybackEngine implements PlaybackEngine
    {
        ...
        @Override
        public PlaybackStatus start(com.fupfin.midiraja.ui.PlaybackUI ui)
        {
            return exitStatus;
        }
        ...
        @Override
        public void addPlaybackEventListener(com.fupfin.midiraja.ui.PlaybackEventListener listener)
        {
        }
        ...
        @Override
        public void setShuffleCallback(java.util.function.Consumer<Boolean> callback)
        {
        }
    }
    ...
    private static MockPlaybackEngine mockEngine(Sequence seq, MidiOutProvider p, PlaylistContext ctx,
            PlaybackPipeline pipeline, java.util.function.BooleanSupplier isShuttingDown,
            double speed, Optional<Long> start, PlaybackStatus exitStatus)
    {
        return new MockPlaybackEngine(seq, p, ctx, pipeline, isShuttingDown, speed, start, exitStatus);
    }

    // ── static helpers ────────────────────────────────────────────────────────

    @Test
    void buildPlayOrder_notShuffled_isSequential()
    {
        assertArrayEquals(new int[] { 0, 1, 2, 3 }, PlaylistPlayer.buildPlayOrder(4, false));
    }

    @Test
    void buildPlayOrder_shuffled_containsAllIndices()
    {
    ...
    @Test
    void buildPlayOrder_sizeZero_returnsEmpty()
    {
    ...
    @Test
    void buildPlayOrder_sizeOne_returnsSingleElement()
    {
        assertArrayEquals(new int[] { 0 }, PlaylistPlayer.buildPlayOrder(1, true));
    }

    @Test
    void reshuffleRemaining_shuffleOn_fullArrayShuffled()
    {
    ...
    @Test
    void reshuffleRemaining_shuffleOff_restoresAscendingOrder()
    {
        int[] order = { 3, 4, 0, 1, 2 };
    ...
    @Test
    void reshuffleRemaining_atLastTrack_currentSongStaysAtEnd()
    {
        int[] order = { 0, 1, 2 };
        PlaylistPlayer.reshuffleRemaining(order, 2, true);
        assertEquals(2, order[2]);
    ...
    @Test
    void reshuffleRemaining_idempotentSortOff()
    {
        int[] order = { 0, 1, 2, 3 };
    ...
    void quitAll_exitsLoopAfterOneEngineCall(@TempDir Path tempDir) throws Exception
    {
        ...
        PlaybackEngineFactory factory = (seq, p, ctx, pipeline, isShuttingDown, speed, start) -> {
            ...
            return mockEngine(seq, p, ctx, pipeline, isShuttingDown, speed, start, PlaybackStatus.QUIT_ALL);
        };
    ...
    void finishedStatus_advancesThroughAllTracks(@TempDir Path tempDir) throws Exception
    {
        ...
        PlaybackEngineFactory factory = (seq, p, ctx, pipeline, isShuttingDown, speed, start) -> {
            callCount.incrementAndGet();
            return mockEngine(seq, p, ctx, pipeline, isShuttingDown, speed, start, PlaybackStatus.FINISHED);
        };
    ...
    void nextStatus_exitsAtNavBoundary(@TempDir Path tempDir) throws Exception
    {
        ...
        PlaybackEngineFactory factory = (seq, p, ctx, pipeline, isShuttingDown, speed, start) -> {
            callCount.incrementAndGet();
            return mockEngine(seq, p, ctx, pipeline, isShuttingDown, speed, start, PlaybackStatus.NEXT);
        };
    ...
    void loopEnabled_wrapsBackToFirstTrack(@TempDir Path tempDir) throws Exception
    {
        ...
        PlaybackEngineFactory factory = (seq, p, ctx, pipeline, isShuttingDown, speed, start) -> {
            ...
            return mockEngine(seq, p, ctx, pipeline, isShuttingDown, speed, start, status);
        };
        common.loop = true;
        player(factory).play(List.of(f1, f2, f3), new MockMidiProvider(),
                new MidiPort(0, "Mock"), common, new DumbUI(), mockIO,
