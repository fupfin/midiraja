...
class BeepSynthProviderTest
{
    ...
    void testAllArchitecturalCombinations() throws Exception
    {
        ...
        for (String synth : synths)
        {
            for (String mux : muxes)
            {
                ...
                {
                    BeepSynthProvider provider = new BeepSynthProvider(
                            new com.fupfin.midiraja.dsp.FloatToShortSink(mockAudio, 1), voices, 1.0, 1.1, 32, mux,
                            synth);
                    provider.openPort(0);
    ...
    {
        BeepSynthProvider provider = new BeepSynthProvider(new com.fupfin.midiraja.dsp.FloatToShortSink(mockAudio, 1),
                2, 1.0, 1.1, 32, "dsd", "fm");
        assertEquals(1, provider.getOutputPorts().size());
        assertEquals("[8-Unit] 1-Bit Digital Cluster", provider.getOutputPorts().get(0).name());

        BeepSynthProvider providerMax = new BeepSynthProvider(
                new com.fupfin.midiraja.dsp.FloatToShortSink(mockAudio, 1), 4, 1.0, 1.1, 32, "dsd", "fm");
        assertEquals("[4-Unit] 1-Bit Digital Cluster", providerMax.getOutputPorts().get(0).name());

        BeepSynthProvider providerMin = new BeepSynthProvider(
                new com.fupfin.midiraja.dsp.FloatToShortSink(mockAudio, 1), 1, 1.0, 1.1, 32, "dsd", "fm");
    ...
    {
        BeepSynthProvider provider = new BeepSynthProvider(new com.fupfin.midiraja.dsp.FloatToShortSink(mockAudio, 1),
                10, -5.0, 999.0, 0, "UNKNOWN", "INVALID");
