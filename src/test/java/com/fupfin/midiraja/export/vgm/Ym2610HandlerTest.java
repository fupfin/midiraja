...
class Ym2610HandlerTest
{
    ...
    @Test
    void export_ym2610ClockPatched() throws Exception
    {
        var seq = new Sequence(Sequence.PPQ, 480);
        seq.createTrack();
        var out = new ByteArrayOutputStream();
        composite().export(seq, out);
        byte[] data = out.toByteArray();
        assertEquals(VgmWriter.YM2610_CLOCK, readInt32Le(data, 0x4C));
    }
    ...
    void initSilence_writesKeyOffFor6Channels() throws Exception
    {
        ...
        // key-off = 0x58 0x28 0xXX where upper nibble = 0 (no ops), lower nibble = ch_addr
        List<int[]> port0Writes = findOpnbPort0Writes(data, 0x80);
    ...
    @Test
    void noteOn_producesOpnbWrites() throws Exception
    {
        var seq = new Sequence(Sequence.PPQ, 480);
        seq.createTrack().add(
                new MidiEvent(new ShortMessage(ShortMessage.NOTE_ON, 0, 69, 100), 0));
        ...
        List<int[]> writes = findOpnbPort0Writes(data, 0x80);
        assertFalse(writes.isEmpty(), "Expected OPNB write commands (0x58) for note on");
    }

    @Test
    void noteOn_usesOpnbCommandsNotYm2612() throws Exception
    {
        var seq = new Sequence(Sequence.PPQ, 480);
        seq.createTrack().add(
                new MidiEvent(new ShortMessage(ShortMessage.NOTE_ON, 0, 69, 100), 0));
        ...
        assertFalse(hasYm2612, "YM2610 must use 0x58/0x59 commands, not YM2612's 0x52/0x53");
    }
    ...
    void voiceStealing_doesNotCrash() throws Exception
    {
        var seq = new Sequence(Sequence.PPQ, 480);
        Track track = seq.createTrack();
        for (int i = 0; i < 8; i++) // more than 6 melodic slots
    ...
    private static List<int[]> findOpnbPort0Writes(byte[] data, int start)
    {
