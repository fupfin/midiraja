...
class OpnMidiSynthProviderTest {
    ...
    static class MockOpnBridge implements OpnMidiNativeBridge {
        ...
        @Override public void init(int sampleRate) { initialized = true; }
        @Override public void loadBankFile(String path) { bankFileLoaded = path; }
        @Override public void loadBankData(byte[] data) { bankDataLoaded = true; }
        @Override public void setNumChips(int numChips) { numChipsSet = numChips; }
        @Override public void switchEmulator(int emulatorId) {}
        @Override public void reset() { resetCalled = true; }
        @Override public void panic() { panicCalled = true; }
        ...
        @Override
        public void generate(short[] buffer, int stereoFrames) { generateCallback.accept(buffer); }
    ...
    void testLifecycleAndMidiRouting() throws Exception {
        ...
        assertTrue(bridge2.bankDataLoaded, "Empty path should load bundled GM bank via loadBankData");
    ...
    void testPrepareForNewTrack() throws Exception {
        ...
        assertTrue(mockBridge.resetCalled,
            "bridge.reset() should be called in prepareForNewTrack");

        // Stale events should have been cleared (not dispatched)
        provider.flushEventQueueForTest();
        assertTrue(mockBridge.eventLog.isEmpty(), "Stale events should be cleared by prepareForNewTrack");
    }
    ...
    void testOutputPorts() {
        ...
        assertTrue(ports.get(0).name().contains("MAME YM2612"), "Default emulator should be MAME YM2612");
