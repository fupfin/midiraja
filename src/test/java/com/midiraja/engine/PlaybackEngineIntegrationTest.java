...
import com.midiraja.midi.MuntNativeBridge;
import com.midiraja.midi.MuntSynthProvider;
...
class PlaybackEngineIntegrationTest {
    ...
    static class CountingMuntBridge implements MuntNativeBridge {
        ...
        @Override public void playNoteOn(int channel, int key, int velocity) { noteOnCount++; }
        @Override public void playNoteOff(int channel, int key)               { noteOffCount++; }
        @Override public void playControlChange(int channel, int number, int value) { ccCount++; }
        @Override public void playProgramChange(int channel, int program)     { pcCount++; }
        @Override public void playPitchBend(int channel, int value)           { pbCount++; }
    }
    ...
    void testAllEventsDelivered() throws Exception {
        ...
        PlaylistContext ctx = new PlaylistContext(
            List.of(new File("monkey_island.mid")), 0,
            new MidiPort(0, "Test"), null);

        PlaybackEngine engine = new PlaybackEngine(
            seq, provider, ctx, 100, 1000.0,
        ...
        // 5. After playback, provider.panic() sends all-notes-off to MuntSynthProvider.sendMessage(),
        //    which routes them through the counting bridge. Account for these in expectations:
        //      16 channels × 4 CC messages (sustain-off, all-notes-off, all-sound-off, reset-CC) = 64
        //      16 channels × 128 note-off messages = 2048
        ...
        assertEquals(expectedNoteOff + PANIC_NOTEOFF_COUNT, bridge.noteOffCount, "NoteOff count mismatch");
