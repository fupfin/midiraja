 ...
 * mechanism. Seven OKI ADPCM drum samples (extracted from FluidR3_GM.sf3 at build time by
 * {@code scripts/extract_drum_samples.py}) are embedded as PCM data blocks (type 0x04) at
 * {@link #initSilence} time, then triggered individually via DAC stream play-block commands
 * (0x95) on each percussion hit.
 *
 * <p>
...
final class Msm6258Handler implements ChipHandler
{
    ...
    /**
     * OKI ADPCM drum samples loaded from classpath resources.
     * Each {@code msm6258_drum_N.bin} was extracted from FluidR3_GM.sf3 at build time
     * and resampled/encoded to OKI ADPCM 4-bit at {@link VgmWriter#MSM6258_SAMPLE_RATE}.
     */
    ...
    public void initSilence(VgmWriter w)
    {
        ...
        // Byte-feed rate: sample rate / 2 because each packed byte holds 2 nibbles (2 samples).
        // Writing a new byte to reg 0x01 resets nibble_shift=0, so if we write at 15625 Hz the
        // high nibble is overwritten before the chip can process it.  At half the rate the chip
        // consumes low-nibble on tick N and high-nibble on tick N+1 before the next byte arrives.
        w.writeDacStreamFrequency(STREAM_ID, VgmWriter.MSM6258_SAMPLE_RATE / 2);
    ...
    /**
     * Loads 7 MSM6258 drum samples from classpath resources.
    ...
    private static byte[][] loadDrumSamples()
    {
        ...
        for (int i = 0; i < 7; i++)
        {
            String name = "msm6258_drum_" + i + ".bin";
            try (var in = Msm6258Handler.class.getResourceAsStream(name))
            {
                if (in == null)
