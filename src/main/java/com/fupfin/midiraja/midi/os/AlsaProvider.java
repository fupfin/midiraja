...
public class AlsaProvider implements MidiOutProvider {
  ...
  @Nullable
  private static final MethodHandle snd_seq_close =
      getMH("snd_seq_close",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS));
  @Nullable
  private static final MethodHandle snd_seq_create_simple_port =
      getMH("snd_seq_create_simple_port",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS,
                                  ...
                                  ValueLayout.JAVA_INT));
  @Nullable
  private static final MethodHandle snd_seq_connect_to =
      getMH("snd_seq_connect_to",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS,
  ...
  private static final MethodHandle snd_seq_client_info_malloc =
      getMH("snd_seq_client_info_malloc",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS));
  @Nullable
  private static final MethodHandle snd_seq_client_info_free =
      getMH("snd_seq_client_info_free",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS));
  @Nullable
  private static final MethodHandle snd_seq_query_next_client =
      getMH("snd_seq_query_next_client",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS,
                                  ValueLayout.ADDRESS));
  @Nullable
  ...
  // Port Info
  @Nullable
  private static final MethodHandle snd_seq_port_info_malloc =
      getMH("snd_seq_port_info_malloc",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS));
  @Nullable
  private static final MethodHandle snd_seq_port_info_free =
      getMH("snd_seq_port_info_free",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS));
  @Nullable
  private static final MethodHandle snd_seq_query_next_port =
      getMH("snd_seq_query_next_port",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS,
                                  ValueLayout.ADDRESS));
  @Nullable
  private static final MethodHandle snd_seq_port_info_set_client =
      getMH("snd_seq_port_info_set_client",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS,
                                  ValueLayout.JAVA_INT));
  @Nullable
  private static final MethodHandle snd_seq_port_info_set_port =
      getMH("snd_seq_port_info_set_port",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS,
                                  ValueLayout.JAVA_INT));
  @Nullable
  ...
  // MIDI Event Parser
  @Nullable
  private static final MethodHandle snd_midi_event_new =
      getMH("snd_midi_event_new",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.JAVA_INT,
                                  ValueLayout.ADDRESS));
  @Nullable
  private static final MethodHandle snd_midi_event_free =
      getMH("snd_midi_event_free",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS));
  @Nullable
  private static final MethodHandle snd_midi_event_encode =
      getMH("snd_midi_event_encode",
            FunctionDescriptor.of(ValueLayout.JAVA_LONG, ValueLayout.ADDRESS,
                                  ValueLayout.ADDRESS, ValueLayout.JAVA_LONG,
                                  ValueLayout.ADDRESS));
  @Nullable
  private static final MethodHandle snd_seq_event_output_direct =
      getMH("snd_seq_event_output_direct",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS,
                                  ValueLayout.ADDRESS));

  // Drain output
  @Nullable
  private static final MethodHandle snd_seq_drain_output =
      getMH("snd_seq_drain_output",
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS));
  ...
  public List<MidiPort> getOutputPorts() {
    ...
    try (Arena arena = Arena.ofConfined()) {
      ...
      if ((int)snd_seq_open.invokeExact(seqRef, nameStr, SND_SEQ_OPEN_OUTPUT,
                                        0) < 0) {
      ...
      if (snd_seq_client_info_malloc != null) {
        int _m1 = (int)snd_seq_client_info_malloc.invokeExact(cInfoRef);
      }
      if (snd_seq_port_info_malloc != null) {
        int _m2 = (int)snd_seq_port_info_malloc.invokeExact(pInfoRef);
      }
      ...
      while ((int)snd_seq_query_next_client.invokeExact(seq, cInfo) == 0) {
        ...
        String clientName =
            clientNamePtr.reinterpret(Long.MAX_VALUE).getString(0);

        if (snd_seq_port_info_set_client != null) {
          int _dummy1 = (int)snd_seq_port_info_set_client.invoke(pInfo, client);
        }
        if (snd_seq_port_info_set_port != null) {
          int _dummy2 = (int)snd_seq_port_info_set_port.invoke(pInfo, -1);
        }
        ...
        while ((int)snd_seq_query_next_port.invokeExact(seq, pInfo) == 0) {
          ...
          if ((caps & PORT_CAP_MASK) == PORT_CAP_MASK) {
            ...
            String portName =
                portNamePtr.reinterpret(Long.MAX_VALUE).getString(0);
            int globalPortIndex = (client << 16) | port;
            ports.add(
                new MidiPort(globalPortIndex, clientName + " - " + portName));
          }
        }
      }
  ...
  public void openPort(int portIndex) throws Exception {
    ...
    try {
      ...
      // SND_SEQ_PORT_CAP_READ | SND_SEQ_PORT_CAP_SUBS_READ (For generic MIDI
      // type 1<<1)
      myPort = (int)snd_seq_create_simple_port.invokeExact(
          seqHandle, portNameStr, (1 << 0) | (1 << 5), (1 << 1));

      int destClient = (portIndex >> 16) & 0xFFFF;
      int destPort = portIndex & 0xFFFF;

      if ((int)snd_seq_connect_to.invokeExact(seqHandle, myPort, destClient,
                                              destPort) < 0) {
      ...
      // Allocate ALSA event structure (64 bytes is safely larger than
      // snd_seq_event_t)
  ...
  public void sendMessage(byte[] data) throws Exception {
    ...
    try {
      ...
      // Encode raw byte[] into the ALSA snd_seq_event_t structure (ev) using
      // the parser
      long processed = (long)snd_midi_event_encode.invokeExact(
          parser, dataSeg, (long)data.length, ev);

      if (processed > 0) {
        // Source port
        ev.set(ValueLayout.JAVA_BYTE, 4,
               (byte)myPort); // event.source.port offset
        // Dest port
        ev.set(ValueLayout.JAVA_BYTE, 6,
               (byte)253); // SND_SEQ_ADDRESS_SUBSCRIBERS offset
