...
public class FluidSynthProvider implements SoftSynthProvider {
  ...
  public FluidSynthProvider(@Nullable String explicitDriver) throws Exception {
    ...
    try {
      ...
      throw new Exception(
          "FluidSynth native library not found! Please install it (e.g., "
              + "'brew install fluidsynth' on Mac). " + e.getMessage(),
          e);
    }
    ...
    fluid_settings_setstr = linker.downcallHandle(
        lib.find("fluid_settings_setstr")
            .orElseThrow(
                () -> new Exception("fluid_settings_setstr not found")),
    ...
    new_fluid_audio_driver = linker.downcallHandle(
        lib.find("new_fluid_audio_driver")
            .orElseThrow(
                () -> new Exception("new_fluid_audio_driver not found")),
        FunctionDescriptor.of(ValueLayout.ADDRESS, ValueLayout.ADDRESS,
                              ValueLayout.ADDRESS));
    ...
    fluid_synth_program_change_mh = linker.downcallHandle(
        lib.find("fluid_synth_program_change")
            .orElseThrow(
                () -> new Exception("fluid_synth_program_change not found")),
        FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS,
                              ValueLayout.JAVA_INT, ValueLayout.JAVA_INT));
    ...
    fluid_synth_pitch_bend_mh = linker.downcallHandle(
        lib.find("fluid_synth_pitch_bend")
            .orElseThrow(
                () -> new Exception("fluid_synth_pitch_bend not found")),
        FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS,
                              ValueLayout.JAVA_INT, ValueLayout.JAVA_INT));
    ...
    fluid_set_log_function = linker.downcallHandle(
        lib.find("fluid_set_log_function")
            .orElseThrow(
                () -> new Exception("fluid_set_log_function not found")),
    ...
    delete_fluid_audio_driver = linker.downcallHandle(
        lib.find("delete_fluid_audio_driver")
            .orElseThrow(
                () -> new Exception("delete_fluid_audio_driver not found")),
    ...
    delete_fluid_settings = linker.downcallHandle(
        lib.find("delete_fluid_settings")
            .orElseThrow(
                () -> new Exception("delete_fluid_settings not found")),
        FunctionDescriptor.ofVoid(ValueLayout.ADDRESS));
  }
  ...
  public void openPort(int portIndex) throws Exception {
    ...
    try {
      ...
      if (explicitDriver != null && !explicitDriver.isEmpty() &&
        ...
        int _dummy =
            (int)fluid_settings_setstr.invokeExact(settings, keyStr, valStr);
      }
      ...
      if (new_fluid_audio_driver != null) {
        adriver =
            (MemorySegment)new_fluid_audio_driver.invokeExact(settings, synth);
      }
  ...
  public void panic() {
    ...
    for (int ch = 0; ch < 16; ch++) {
      try {
        ...
        sendMessage(
            new byte[] {(byte)(0xB0 | ch), 121, 0}); // Reset All Controllers
