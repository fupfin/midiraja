 ...
 *
 * <p>
 * The HuC6280 has 6 wavetable channels with stereo volume control. VGM command 0xB9
 ...
 *
 * <p>
 * <b>Waveform analysis:</b> Each channel has a 32-sample, 5-bit wavetable written via
...
public class HuC6280MidiConverter
{
    ...
    public void convert(VgmEvent event, Track[] tracks, long clock, long tick)
    {
        ...
        switch (reg)
        {
            ...
            default ->
                {
                } // 0x01 master vol, 0x07 noise, 0x08-0x09 LFO
        }
    }
    ...
    {
        if (!enabled[ch] || volume[ch] == 0 || activeNote[ch] < 0)
            return;
        int note = computeNote(ch, clock);
        if (note != activeNote[ch])
        {
