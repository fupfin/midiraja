 ...
 * </ul>
 *
 * <p>VGM command 0xB3 carries a register offset (0x00-0x3F mapping to NR10-NR52 + Wave RAM)
 * and a data byte. Register offsets are relative to 0xFF10 in the Game Boy memory map.
 *
 * <p><b>Frequency encoding:</b> Pulse and wave channels use an 11-bit frequency register
 * where f = clock / (32 × (2048 - freq)). The DMG clock for audio is typically 4,194,304 Hz.
 *
 * <p><b>Volume:</b> NR12/NR22 bits 7-4 = initial volume (0-15), bit 3 = direction.
 * NR32 bits 6-5 = wave output level (0=mute, 1=100%, 2=50%, 3=25%).
 * NR42 bits 7-4 = initial volume for noise.
 *
 * <p><b>Trigger:</b> Writing bit 7 of NR14/NR24/NR34/NR44 restarts the channel (trigger event).
 */
public class GameBoyDmgMidiConverter {
    ...
    public void convert(VgmEvent event, Track[] tracks, long clock, long tick) {
        ...
        switch (reg) {
            ...
            // Master
            case 0x15 -> { panRegister = data; updateAllPans(tick, tracks); } // NR51: L/R panning

            default -> {} // NR10 sweep, NR20/NR40 unused, wave RAM, NR50/NR52
        }
    }
    ...
    private void emitPanForChannel(int ch, long tick, Track[] tracks) {
        ...
        int pan;
        if (left && right) pan = 64;
        else if (left) pan = 0;
        else if (right) pan = 127;
        else pan = 64;
    ...
    private void updateAllPans(long tick, Track[] tracks) {
        for (int ch = 0; ch < 4; ch++) {
            if (activeNote[ch] >= 0) emitPanForChannel(ch, tick, tracks);
        }
    }
