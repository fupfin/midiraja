 ...
 *
 * <p>
 * Frequency formula: {@code f = clock / (16 × N)} where N is the 12-bit tone period.
 *
 * <p>
 * <b>Fine + coarse writes both trigger pitch retrigger:</b> MSX games implementing
 ...
 *
 * <p>
 * <b>Noise → hi-hat only:</b> The AY noise generator is an LFSR producing electrical
 ...
 *
 * <p>
 * <b>Envelope mode approximation:</b> The AY-3-8910 envelope generator (R13) modulates
 ...
 *
 * <p>
 * <b>Volume → CC7 via square-root curve:</b> Linear mapping ({@code vol/15×127}) places
...
public class Ay8910MidiConverter
{
    ...
    public void convert(VgmEvent event, Track[] tracks, long clock, long tick)
    {
        ...
        switch (reg)
        {
            ...
            case 0 ->
            {
                period[0] = (period[0] & 0xF00) | val;
                handleTone(0, tick, tracks, clock);
            }
            case 1 ->
            {
                period[0] = (period[0] & 0x0FF) | ((val & 0x0F) << 8);
                handleTone(0, tick, tracks, clock);
            }
            case 2 ->
            {
                period[1] = (period[1] & 0xF00) | val;
                handleTone(1, tick, tracks, clock);
            }
            case 3 ->
            {
                period[1] = (period[1] & 0x0FF) | ((val & 0x0F) << 8);
                handleTone(1, tick, tracks, clock);
            }
            case 4 ->
            {
                period[2] = (period[2] & 0xF00) | val;
                handleTone(2, tick, tracks, clock);
            }
            case 5 ->
            {
                period[2] = (period[2] & 0x0FF) | ((val & 0x0F) << 8);
                handleTone(2, tick, tracks, clock);
            }
    ...
    private void handleTone(int ch, long tick, Track[] tracks, long clock)
    {
        ...
        if (volume[ch] == 0 || !toneEn[ch] || activeNote[ch] < 0)
            return;
    ...
    private void updateNoise(long tick, Track[] tracks)
    {
        ...
        for (int ch = 0; ch < 3; ch++)
        {
            if (noiseEn[ch] && volume[ch] > maxVol)
                maxVol = volume[ch];
        }
     ...
     *
     * <p>
     * The AY-3-8910 noise generator is an LFSR producing electrical white noise — there is no
     ...
     *
     * <p>
     * Noise frequency = clock / (16 × period). Higher period → slower LFSR → coarser noise:
     * <ul>
     ...
     *
     * <p>
     * The AY-3-8910 envelope generator modulates amplitude over time; R13 selects the shape:
     * <ul>
