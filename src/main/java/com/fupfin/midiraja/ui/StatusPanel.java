...
public class StatusPanel implements Panel
{
    ...
    public void render(ScreenBuffer buffer)
    {
        ...
        {
            String s = String.format("    %s%s / %s %s %3d%%  Vol:%d%% Spd:%.1fx Tr:%+d",
                    pauseIndicator, curStr, totStr, bar, percent, (int) (volumeScale * 100), speed,
                    transpose);
            buffer.append(truncate(s, constraints.width())).append("\n");
        }
        else if (constraints.height() >= 2 && constraints.height() <= 4)
        {
            String line1 = String.format("    Time: %s%s / %s  %s  %3d%%", pauseIndicator, curStr,
                    totStr, bar, percent);
            ...
            for (int i = 2; i < constraints.height(); i++)
                buffer.append("\n");
        }
        else
        {
