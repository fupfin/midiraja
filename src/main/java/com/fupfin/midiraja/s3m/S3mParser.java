 ...
 *
 * <p>Binary layout (all multi-byte integers are little-endian):
 * <ul>
 ...
 * </ul>
 *
 * <p>S3M note encoding: {@code (octave << 4) | semitone} → MIDI note = {@code octave * 12 + semitone}.
...
public class S3mParser
{
    ...
    public S3mParseResult parseBytes(byte[] data) throws IOException
    {
        ...
        for (int i = 0; i < 32; i++)
        {
            ...
            if (chanSettings[i] != 0xFF && chanSettings[i] < 16) channelCount++;
        }
        ...
        for (int i = 0; i < 32 && idx < 16; i++)
            if (chanSettings[i] != 0xFF && chanSettings[i] < 16) chanIndexMap[i] = idx++;

        // Orders
        int orderOffset = 96;
        int[] orders = new int[ordNum];
        for (int i = 0; i < ordNum; i++) orders[i] = data[orderOffset + i] & 0xFF;
            ...
            int[] chanIndexMap, int channelCount, int initSpeed, int initTempo)
    {
        ...
        while (orderPos < ordNum)
        {
            ...
            if (orderVal == 0xFF) break; // end marker
            if (orderVal == 0xFE) { orderPos++; row = 0; continue; } // skip marker

            long key = ((long) orderPos << 16) | row;
            if (!visited.add(key)) break;

            int patIdx = orderVal;
            if (patIdx >= patPara.length || patPara[patIdx] == 0) { orderPos++; row = 0; continue; }

            int patOffset = patPara[patIdx];
            if (patOffset + 2 > data.length) { orderPos++; row = 0; continue; }
            ...
            for (int ch = 0; ch < 32; ch++)
            {
                ...
                if (fx == FX_SET_SPEED && pa > 0)  nextSpeed = pa;
                if (fx == FX_SET_TEMPO && pa >= 32) nextTempo = pa;
            }
            ...
            for (int ch = 0; ch < 32; ch++)
            {
                int fx = rowFxCmd[row][ch];
                int pa = rowFxPar[row][ch];
                if (fx == FX_PAT_JUMP)  jumpToOrder = pa;
                if (fx == FX_PAT_BREAK) breakToRow  = ((pa >> 4) * 10) + (pa & 0x0F);
            }
            ...
            {
                row++;
                if (row >= ROWS_PER_PATTERN) { row = 0; orderPos++; }
            }
        }
