...
/**
 * Converts an {@link XmParseResult} into a {@link javax.sound.midi.Sequence}.
 *
 * <p>Channel mapping: XM channels are mapped to MIDI channels 0–14, skipping channel 9 (GM
 * drums). Instruments whose name matches a percussion keyword are routed to channel 9.
 *
 * <p>Timing: fixed 120 BPM (PPQ=480); tick = round(microsecond × 960 / 1_000_000).
 */
public class XmToMidiConverter
{
    ...
    public XmToMidiConverter()               { this(Set.of()); }
    public XmToMidiConverter(Set<Integer> m) { this.mutedChannels = m; }
    ...
    public Sequence convert(XmParseResult parsed)
    {
        try
        {
            ...
            {
                int xmCh = ev.channel();
                if (xmCh >= maxCh) continue;

                long tick  = toTick(ev.microsecond());
                int midiCh = midiChannel[xmCh];
                ...
                if (ev.instrument() > 0 && ev.instrument() <= insCount)
                {
                    ...
                    if (instrIsDrum[i])
                    {
                        isDrum[xmCh] = true;
                        midiCh = DRUM_CH;
                        midiChannel[xmCh] = DRUM_CH;
                    }
                    else if (activeInstr[xmCh] != i)
                    {
                        isDrum[xmCh] = false;
                ...
                if (note == -2) // key-off
                {
                    if (activeNote[xmCh] >= 0)
                        addEvent(routed[midiCh], ShortMessage.NOTE_OFF, midiCh, activeNote[xmCh], 0, tick);
                    activeNote[xmCh] = -1;
                }
                else if (note >= 0)
                {
                    ...
                    int instrIdx = ev.instrument() > 0 ? ev.instrument() : activeInstr[xmCh];
                    ...
                    int destCh = isDrum[xmCh] ? DRUM_CH : midiCh;
                    addEvent(routed[destCh], ShortMessage.NOTE_ON, destCh, note, vel, tick);
                    activeNote[xmCh] = note;
                }

                // Volume slide effect (0xA)
                if (ev.effectCmd() == 0x0A)
                {
                    int up   = (ev.effectParam() >> 4) & 0x0F;
                    int down = ev.effectParam() & 0x0F;
                    if (up > 0 || down > 0)
                    {
                        int instrIdx = activeInstr[xmCh] > 0 ? activeInstr[xmCh] : 0;
                        int baseVol = instrIdx > 0 && instrIdx <= insCount
                                ? instruments.get(instrIdx - 1).volume() : 64;
    ...
    /** Maps XM channel indices to MIDI channels, skipping channel 9. */
