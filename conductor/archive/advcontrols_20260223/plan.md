...
- [x] Task: Define new CLI options in `MidirajaCommand.java`. bae1e4c
    - [ ] Write Tests: Create `MidirajaCommandTest` to verify that `--volume` and `--transpose` arguments are correctly parsed.
    - [ ] Implement: Add `@Option` for volume and transpose in `MidirajaCommand.java`.
...
- [x] Task: Integrate volume control into playback flow. cc22ff1
    ...
    - [ ] Implement: Update `MidirajaCommand.playMidiWithProvider` to call volume initialization.
...
- [x] Task: Implement note transposition logic in the playback loop. 8ed1b92
    ...
    - [ ] Implement: Modify the message processing logic in `MidirajaCommand.playMidiWithProvider` to apply the transpose offset to Note On/Off messages.
